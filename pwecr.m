function [mean_ed,mean_ecu, mean_ecr, mean_fcr] = pwnoecr(mvced, mvcecu, mvcecr, mvcfcr, r1, r2, r3,m,n)
...
% does not take into account second rep of ECR muscle
...
% n to the number of figure 
    ...
    ecrmax = max([ecrmvc ecr_1_m ecr_3_m]);
    ...
    elseif ecr_1_m == ecrmax
        mean_ecr = sqrt(mean(1/ecrmax*[ecr_3_m]).^2)
        signal_ecr = (ecr_3)/(1*ecrmax);
        ecr_plot = sqrt(movmean(signal_ecr.^2,windowlen));
